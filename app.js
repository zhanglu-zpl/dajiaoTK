...
const noteModels = noteModelSpecs.map(([title, category, page], index) => ({
  id:`note-model-${index+1}`, no:`M${index+1}`, title, category, scan:true, book:'《中考几何满分冲刺》', shape:'书本模型页', method:'整页扫描', source:'model-scan', page, answerPages:[{page, image:`assets/note-models/page-${String(page).padStart(3,'0')}.jpg`}], thumb:`assets/note-models/page-${String(page).padStart(3,'0')}.jpg`, note:'《中考几何满分冲刺》书本扫描页，模型讲解与例题在同一页。'
}));
// 《几何模型 · 考前必看六类模型》书本扫描页（全册 6 个内容页，按册内顺序排列，原 PDF 共 8 页含封面封底）。
// 当前环境无法对扫描图做可靠文字识别，先按页序编号命名、归入“综合模型”（跨专题冲刺资料），
// 每页同时展示在“全部题型”与“综合模型”下；核对每页内容后，可把标题改为精确模型名并调整分类。
const sixModelSpecs = [
  ['考前必看模型（一）',1],['考前必看模型（二）',2],['考前必看模型（三）',3],
  ['考前必看模型（四）',4],['考前必看模型（五）',5],['考前必看模型（六）',6]
];
const sixModels = sixModelSpecs.map(([title, page], index) => ({
  id:`six-model-${index+1}`, no:`S${index+1}`, title, category:'comprehensive', scan:true, book:'《几何模型 · 考前必看六类模型》', shape:'书本模型页', method:'整页扫描', source:'six-model-scan', page, answerPages:[{page, image:`tmp/pdfs/notes-pages/page-${String(page).padStart(3,'0')}.png`}], thumb:`tmp/pdfs/notes-pages/page-${String(page).padStart(3,'0')}.png`, note:'《几何模型 · 考前必看六类模型》书本扫描页，模型讲解与例题在同一页。'
}));
// 最新添加的《考前必看六类模型》扫描页排在最前，打开“全部题型”即可看到；其后是 49 个书本模型页与 14 道辅助线原题。
const allQuestions = sixModels.concat(noteModels, questions);
const sources = [
...
function renderTopics(){ const q=search.value.trim().toLowerCase(); const list=allQuestions.filter(t=>inCategory(t,activeCategory)&&(!q||(t.title+t.shape+t.method+t.note).toLowerCase().includes(q))); grid.innerHTML=list.length?list.map(t=>`<article class="question-card" data-question="${t.id}"><div class="question-thumb"><img src="${t.thumb}" alt="${t.title}" loading="lazy"/><span class="question-no">${String(t.no).padStart(2,'0')}</span></div><div class="question-card-content"><div class="topic-top"><span class="shape-label">${t.scan?`${categoryLabel(t.category)} · 模型`:t.shape}</span><span class="topic-count">${t.scan?'扫描页':'第 '+t.no+' 题'}</span></div><h3>${t.title}</h3><p>${t.scan?'点击查看书本模型页（扫描件）':'点击查看题目与解析答案'}</p><div class="topic-footer"><span>查看详情</span><span class="topic-arrow">→</span></div></div></article>`).join(''):`<div class="empty-reader" style="position:relative;grid-column:1/-1;padding:60px 0"><h4>没有找到匹配题目</h4><p>请换一个关键词搜索。</p></div>`; document.querySelector('#sectionTitle').textContent=categories.find(category=>category.id===activeCategory).label.replace('题型','题目'); }
...
function openQuestion(q){currentQuestion=q;answerIndex=0;const answers=answerPages(q);const isScan=q.scan;questionTitle.textContent=isScan?q.title:`第 ${q.no} 题 · ${q.title}`;const metaText=isScan?`${q.book} 书本扫描页 · 原书第 ${q.page} 页`:answers.length?`原题第 ${q.page} 页 · 已配置解析 ${answers.map(answer=>answer.page).join('、')} 页`:`原题第 ${q.page} 页 · 解析第 20-33 页，请自行翻找`;questionMeta.textContent=metaText;questionSource.textContent=isScan?'模型讲解与例题在同一页，可直接翻看':answers.length?'可使用页码框和上下按钮查找解析':metaText;questionImage.src=q.thumb;questionImage.classList.remove('hidden-answer');answerImage.classList.add('hidden-answer');showAnswer.textContent='查看解析答案';answerPageInput.value=answers.length?(answers[0].page):20;renderAnswerPage();questionModal.classList.remove('hidden');questionModal.setAttribute('aria-hidden','false');}
