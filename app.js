const categories = [
  ...
  // 新增题型：《中考几何满分冲刺-手写笔记》中的模型页单独成一个入口，
  // 便于“按模型查题”；同一个模型仍保留原来的学科分类（见 noteModels.categories）。
  { id:'note-models', label:'手写笔记模型', icon:'✎' },
];
const topics = [
...
const noteModels = noteModelSpecs.map(([title, category, page], index) => ({
  id:`note-model-${index+1}`, no:`M${index+1}`, title, category, categories:['note-models', category], shape:'笔记模型', method:'手写笔记模型', source:'note-models', page, answerPages:[{page, image:`assets/note-models/page-${String(page).padStart(3,'0')}.jpg`}], thumb:`assets/note-models/page-${String(page).padStart(3,'0')}.jpg`, note:'来自《中考几何满分冲刺-手写笔记》目录对应模型页。'
}));
...
// 题目的题型归属：题型分类（categories）是主入口；手写笔记模型额外标注学科分类，
// 因此用 categories 数组表达“一个内容可以同时出现在两个题型下”，单值 category 保持兼容。
function inCategory(item, category){ return category === 'all' || (item.categories ? item.categories.includes(category) : item.category === category); }
